# Roadmap

## 0.1 Core Snapshot

- [x] Two-deck card model and parser
- [x] Legal action classification and comparison
- [x] Wildcard support
- [x] Four-player partnership state transitions
- [x] Heuristic baseline and deterministic demo
- [x] Unit tests and GitHub Actions

## Next

- [ ] Add configurable tribute and return-card rules
- [ ] Add more regional rule profiles
- [ ] Publish a machine-readable game record format
- [ ] Add replay visualization
- [ ] Add reproducible baseline evaluation commands
- [ ] Add optional belief-sampling and policy-value examples

Issues and pull requests are welcome for any roadmap item.
